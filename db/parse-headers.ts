...
function generateSchema(columns: Column[]): string {
    ...
    for (const c of columns) {
        ...
        lines.push(`    ${key}: ${builder}(${JSON.stringify(key)})${notNull}, // ${c.name}`);
    }
    return [
...
async function main() {
    ...
    for (const c of singleTypes) {
        ...
        if (scope === "common") {
            ...
            const merged = reduceTypes([c.type as InferredType, d.type as InferredType]);
            console.log(fmt(scope, { ...c, type: merged, nullable: c.nullable || d.nullable }));
        } else {
    ...
    for (const c of singleTypes) {
        if (doubleSet.has(c.name)) {
            ...
            columns.push({ name: c.name, type: c.type, nullable: true, scope: "single" });
        }
    }
    for (const c of doubleTypes) {
        if (!singleSet.has(c.name)) {
            columns.push({ name: c.name, type: c.type, nullable: true, scope: "double" });
        }
    }

    const focused =
        FOCUS_COLUMNS.size === 0 ? columns : columns.filter((c) => FOCUS_COLUMNS.has(c.name));
